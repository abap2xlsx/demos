...
CLASS lcl_xlsx_diff_item DEFINITION
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_diff_item_attr,
    ...
    TYPES: BEGIN OF ty_diff_item,
           ...
           END OF ty_diff_item.
    TYPES ty_diff_items TYPE STANDARD TABLE OF ty_diff_item WITH DEFAULT KEY.
    TYPES: BEGIN OF ty_item,
             ...
             size       TYPE cl_abap_zip=>t_file-size,
           END OF ty_item.
    TYPES ty_items TYPE SORTED TABLE OF ty_item WITH UNIQUE KEY local_name.
    ...
    DATA items        TYPE ty_diff_items READ-ONLY.
    DATA is_different TYPE abap_bool     READ-ONLY.
  ...
  PRIVATE SECTION.
    ...
    DATA zip_2 TYPE REF TO cl_abap_zip.
...
CLASS lcl_xlsx_diff_viewer DEFINITION
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS icon_old TYPE tv_image                  VALUE icon_create.
    CONSTANTS icon_new TYPE tv_image                  VALUE icon_msg.
    ...
    DATA go_tree     TYPE REF TO cl_column_tree_model.
    DATA node_key    TYPE i.
    DATA title_old   TYPE text40.
    DATA title_new   TYPE text40.
    DATA zip_old     TYPE REF TO cl_abap_zip.
    DATA zip_new     TYPE REF TO cl_abap_zip.
    DATA diff_file   TYPE REF TO lcl_xlsx_diff_file_ext.
...
CLASS lcl_app DEFINITION.
  ...
  PRIVATE SECTION.
    ...
    TYPES: BEGIN OF ty_demo,
           ...
           END OF ty_demo.
    TYPES ty_demos TYPE STANDARD TABLE OF ty_demo WITH DEFAULT KEY.
    ...
    TYPES: BEGIN OF ty_alv_line,
           ...
           END OF ty_alv_line.
    TYPES ty_alv_table              TYPE STANDARD TABLE OF ty_alv_line WITH DEFAULT KEY.
    TYPES ty_popup_confirm_question TYPE c LENGTH 400.
    ...
    DATA p_path             TYPE zexcel_export_dir.
    "! Do an exact comparison of XML
    DATA p_exaxml           TYPE abap_bool.
    DATA splitter           TYPE REF TO cl_gui_splitter_container.
    DATA alv_container      TYPE REF TO cl_gui_container.
    DATA zip_diff_container TYPE REF TO cl_gui_container.
    DATA viewer             TYPE REF TO lcl_xlsx_diff_viewer.
    DATA salv               TYPE REF TO cl_salv_table.
    DATA alv_table          TYPE ty_alv_table.
    DATA lv_filesep         TYPE c LENGTH 1.
...
CLASS lcl_zip_cleanup_for_diff IMPLEMENTATION.
  METHOD run.
    ...
    TYPES: BEGIN OF ty_central_file_header,
    ...
    TYPES: BEGIN OF ty_end_of_central_dir,
    ...
    CONSTANTS local_file_header_signature   TYPE x LENGTH 4 VALUE '504B0304'.
    CONSTANTS central_file_header_signature TYPE x LENGTH 4 VALUE '504B0102'.
    CONSTANTS end_of_central_dir_signature  TYPE x LENGTH 4 VALUE '504B0506'.

    DATA local_file_header         TYPE ty_zip_structure.
    DATA dummy_local_file_header   TYPE ty_local_file_header.
    DATA central_file_header       TYPE ty_zip_structure.
    DATA dummy_central_file_header TYPE ty_central_file_header.
    DATA end_of_central_dir        TYPE ty_zip_structure.
    DATA dummy_end_of_central_dir  TYPE ty_end_of_central_dir.
    DATA offset                    TYPE i.
    DATA max_offset                TYPE i.

    FIELD-SYMBOLS <local_file_header>     TYPE ty_local_file_header.
    FIELD-SYMBOLS <local_file_header_x>   TYPE x.
    FIELD-SYMBOLS <central_file_header>   TYPE ty_central_file_header.
    FIELD-SYMBOLS <central_file_header_x> TYPE x.
    FIELD-SYMBOLS <end_of_central_dir>    TYPE ty_end_of_central_dir.
    FIELD-SYMBOLS <end_of_central_dir_x>  TYPE x.

    local_file_header = init_structure( length      = 30
                                        charset_bit = 60
                                        structure   = dummy_local_file_header ).
    ASSIGN local_file_header-ref_to_structure->* TO <local_file_header>.
    ASSIGN local_file_header-ref_to_x->* TO <local_file_header_x>.

    central_file_header = init_structure( length      = 46
                                          charset_bit = 76
                                          structure   = dummy_central_file_header ).
    ...
    end_of_central_dir = init_structure( length      = 22
                                         charset_bit = 0
                                         structure   = dummy_end_of_central_dir ).
    ...
    WHILE offset <= max_offset.
      ...
      CASE result+offset(4).
        ...
        WHEN local_file_header_signature.
          ...
          read_zip( EXPORTING zip_xstring   = result
                              offset        = offset
                    CHANGING  zip_structure = local_file_header ).

          CLEAR <local_file_header>-last_mod_file_date.
          CLEAR <local_file_header>-last_mod_file_time.

          write_zip( EXPORTING offset        = offset
                     CHANGING  zip_structure = local_file_header
                               zip_xstring   = result ).
        ...
        WHEN central_file_header_signature.
          ...
          read_zip( EXPORTING zip_xstring   = result
                              offset        = offset
                    CHANGING  zip_structure = central_file_header ).
  ...
  METHOD init_structure.
    DATA offset      TYPE i.
    DATA rtts_struct TYPE REF TO cl_abap_structdescr.
  ...
  METHOD read_zip.
    ...
    FIELD-SYMBOLS <zip_structure_x> TYPE x.
    FIELD-SYMBOLS <zip_structure>   TYPE any.
    ...
    IF charset = 0.
      IF zip_structure-conv_in_ibm437 IS NOT BOUND.
        zip_structure-conv_in_ibm437 = cl_abap_conv_in_ce=>create( encoding = '1107'
                                                                   endian   = 'L' ).
      ENDIF.
      zip_structure-conv_in_ibm437->convert_struc( EXPORTING input = <zip_structure_x>
                                                             view  = zip_structure-view
                                                   IMPORTING data  = <zip_structure> ).
    ELSE.
      IF zip_structure-conv_in_utf8 IS NOT BOUND.
        zip_structure-conv_in_utf8 = cl_abap_conv_in_ce=>create( encoding = '4110'
                                                                 endian   = 'L' ).
      ENDIF.
      zip_structure-conv_in_utf8->convert_struc( EXPORTING input = <zip_structure_x>
                                                           view  = zip_structure-view
                                                 IMPORTING data  = <zip_structure> ).
    ENDIF.
  ENDMETHOD.
  ...
  METHOD write_zip.
    ...
    IF charset = 0.
      IF zip_structure-conv_out_ibm437 IS NOT BOUND.
        zip_structure-conv_out_ibm437 = cl_abap_conv_out_ce=>create( encoding = '1107'
                                                                     endian   = 'L' ).
      ENDIF.
      zip_structure-conv_out_ibm437->convert_struc( EXPORTING data   = <zip_structure>
                                                              view   = zip_structure-view
                                                    IMPORTING buffer = <zip_structure_x> ).
    ELSE.
      IF zip_structure-conv_out_utf8 IS NOT BOUND.
        zip_structure-conv_out_utf8 = cl_abap_conv_out_ce=>create( encoding = '4110'
                                                                   endian   = 'L' ).
      ENDIF.
      zip_structure-conv_out_utf8->convert_struc( EXPORTING data   = <zip_structure>
                                                            view   = zip_structure-view
                                                  IMPORTING buffer = <zip_structure_x> ).
    ENDIF.
...
CLASS lcl_xlsx_cleanup_for_diff IMPLEMENTATION.
  METHOD run.
    ...
    DATA lo_filter            TYPE REF TO if_ixml_node_filter.
    ...
    DATA zip_cleanup_for_diff TYPE REF TO lcl_zip_cleanup_for_diff.

    FIELD-SYMBOLS <file>     TYPE cl_abap_zip=>t_file.
    FIELD-SYMBOLS <ls_file2> TYPE ty_file.

    CREATE OBJECT zip.
    zip->load( EXPORTING  zip             = xstring
               EXCEPTIONS zip_parse_error = 1
                          OTHERS          = 2 ).
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_excel
        EXPORTING error = 'zip load'.
    ENDIF.
    ...
    LOOP AT zip->files ASSIGNING <file>.
      ...
      IF normalize_xml = abap_true.
        IF    <file>-name CP '*.xml'
           OR <file>-name CP '*.vml'.
          string = cl_abap_codepage=>convert_from( content ).
          TRY.
              CALL METHOD ('ZCL_EXCEL_XML')=>rewrite_xml_via_sxml
                EXPORTING iv_xml_string = string
                RECEIVING rv_string     = string.
              content = cl_abap_codepage=>convert_to( string ).
    ...
    LOOP AT lt_file ASSIGNING <ls_file2>.
      ...
      zip->add( name    = <ls_file2>-name
                content = <ls_file2>-content ).
    ENDLOOP.
...
CLASS lcl_xlsx_diff_item IMPLEMENTATION.
  METHOD compare_items_at_path.
    DATA zip_1_items   TYPE lcl_xlsx_diff_item=>ty_items.
    DATA zip_2_items   TYPE lcl_xlsx_diff_item=>ty_items.
    DATA zip_1_index   TYPE i.
    DATA zip_2_index   TYPE i.
    DATA zip_1_read    TYPE abap_bool.
    DATA zip_2_read    TYPE abap_bool.
    DATA item_1        TYPE REF TO lcl_xlsx_diff_item=>ty_item.
    DATA item_2        TYPE REF TO lcl_xlsx_diff_item=>ty_item.
    DATA folder_diff   TYPE REF TO lcl_xlsx_diff_item.
    DATA diff_state    TYPE lcl_xlsx_diff_item=>ty_diff_state.
    DATA temp_string_1 TYPE string.
    DATA temp_string_2 TYPE string.
    DATA diff_item     TYPE ty_diff_item.
    DATA item          TYPE REF TO lcl_xlsx_diff_item=>ty_item.

    CREATE OBJECT result TYPE lcl_xlsx_diff_item.
    result->path = path.

    zip_1_items = get_items_at_path( items = zip_1->files
                                     path  = path ).
    zip_2_items = get_items_at_path( items = zip_2->files
                                     path  = path ).
...
*          IF contains( val = item_1->local_name end = '/' ).
          ...
          ELSE.
            ...
            diff_state = compare_two_items( zip_1  = zip_1
                                            zip_2  = zip_2
                                            name_1 = temp_string_1
                                            name_2 = temp_string_2 ).
          ENDIF.
        ENDIF.
...
*          folder_diff = folder_diff ).
  ...
  METHOD compare_two_items.
    ...
    DATA content_2 TYPE xstring.

    zip_1->get( EXPORTING  name                    = name_1
                IMPORTING  content                 = content_1
                EXCEPTIONS zip_index_error         = 1
                           zip_decompression_error = 2
                           OTHERS                  = 3 ).
    IF sy-subrc <> 0.
      " todo
    ENDIF.
    zip_2->get( EXPORTING  name                    = name_2
                IMPORTING  content                 = content_2
                EXCEPTIONS zip_index_error         = 1
                           zip_decompression_error = 2
                           OTHERS                  = 3 ).
    IF sy-subrc <> 0.
...
*    result = COND #( WHEN content_1 <> content_2 THEN state-only_content_changed ELSE state-same ).
  ...
  METHOD get_items_at_path.
    ...
    DATA item_or_folder TYPE string VALUE '^[^/]*(?:/|$)'.
    DATA path_item      TYPE lcl_xlsx_diff_item=>ty_item.
    ...
    LOOP AT items ASSIGNING <item>.
      IF path = ``.
        local_name = substring_to( val   = <item>-name
                                   regex = item_or_folder ).
      ELSEIF strlen( <item>-name ) >= strlen( path ) AND substring( val = <item>-name
                                                                    len = strlen( path ) ) = path.
*      ELSEIF contains( val = <item>-name start = path ).
        local_name = substring_to( val   = substring( val = <item>-name
                                                      off = strlen( path ) )
                                   regex = item_or_folder ).
      ENDIF.
...
CLASS lcl_xlsx_diff_viewer IMPLEMENTATION.
  ...
  METHOD diff_and_view.
    ...
    zip_diff = lcl_xlsx_diff_item=>get_diff( zip_1 = zip_old
                                             zip_2 = zip_new ).
  ...
  METHOD view.
    ...
    DATA temp_container      TYPE REF TO cl_gui_container.
    DATA event               TYPE cntl_simple_event.
    DATA events              TYPE cntl_simple_events.
    DATA isfolder            TYPE as4flag.
    DATA item_table          TYPE treemcitab.
    DATA item_line           TYPE treemcitem.
    ...
    IF go_tree IS NOT BOUND.
      ...
      go_splitter->set_row_sash( id    = 1
                                 type  = go_splitter->type_sashvisible
                                 value = go_splitter->false ).

      temp_container = go_splitter->get_container( row    = 1
                                                   column = 1 ).
      CREATE OBJECT go_toolbar
        EXPORTING parent       = temp_container
                  display_mode = cl_gui_toolbar=>m_mode_horizontal.
      go_toolbar->add_button( fcode     = 'TITLE_OLD'
                              icon      = icon_old
                              butn_type = cntb_btype_button
                              text      = title_old ).
      go_toolbar->add_button( fcode     = 'TITLE_NEW'
                              icon      = icon_new
                              butn_type = cntb_btype_button
                              text      = title_new ).

      CREATE OBJECT go_tree
        EXPORTING node_selection_mode   = cl_gui_simple_tree=>node_sel_mode_single
                  hierarchy_column_name = 'C1'
                  hierarchy_header      = ls_hierarchy_header.

      go_tree->add_column( name         = 'DATE_1'
                           width        = 0
                           header_image = icon_old
                           header_text  = 'Date' ).

      go_tree->add_column( name         = 'DATE_2'
                           width        = 0
                           header_image = icon_new
                           header_text  = 'Date' ).

      go_tree->add_column( name         = 'TIME_1'
                           width        = 0
                           header_image = icon_old
                           header_text  = 'Time' ).

      go_tree->add_column( name         = 'TIME_2'
                           width        = 0
                           header_image = icon_new
                           header_text  = 'Time' ).

      go_tree->add_column( name         = 'SIZE_1'
                           width        = 0
                           header_image = icon_old
                           header_text  = 'Size' ).

      go_tree->add_column( name         = 'SIZE_2'
                           width        = 0
                           header_image = icon_new
                           header_text  = 'Size' ).

      temp_container = go_splitter->get_container( row    = 2
                                                   column = 1 ).
      ...
      go_tree->set_registered_events( EXPORTING  events                    = events
...
*                  WITH sy-msgv1 sy-msgv2 sy-msgv3 sy-msgv4.
    ...
    go_tree->add_node( node_key   = '1'
                       isfolder   = isfolder
                       item_table = item_table ).

    node_key = 1.
    add_tree_nodes( diff_object     = i_zip_diff
                    path            = ''
                    parent_node_key = '1' ).

    go_tree->expand_root_nodes( EXCEPTIONS illegal_level_count = 1
                                           OTHERS              = 2 ).
    IF sy-subrc <> 0.
*     MESSAGE ID sy-msgid TYPE sy-msgty NUMBER sy-msgno
*                WITH sy-msgv1 sy-msgv2 sy-msgv3 sy-msgv4.
    ...
    go_tree->adjust_column_width( EXPORTING  all_columns               = abap_true
...
*                  WITH sy-msgv1 sy-msgv2 sy-msgv3 sy-msgv4.
  ...
  METHOD add_tree_nodes.
    DATA diff_item          TYPE REF TO lcl_xlsx_diff_item=>ty_diff_item.
    DATA lt_column          TYPE treemcitab.
    DATA item_line          TYPE treemcitem.
    DATA save_node_key      TYPE i.
    DATA temp_node_key      TYPE string.
    DATA isfolder           TYPE as4flag.
    DATA temp_string        TYPE string.
    DATA temp_save_node_key TYPE string.
...
*        text      = |{ diff_item->attr_2-size }| ) ).
      ...
      ELSE.
        isfolder = abap_false.
      ENDIF.
      go_tree->add_node( node_key          = temp_node_key
      ...
      IF     diff_item->folder_diff                 IS BOUND
        ...
        add_tree_nodes( diff_object     = diff_item->folder_diff
                        path            = temp_string
                        parent_node_key = temp_node_key ).
        IF diff_item->diff_state <> state-same.
          temp_save_node_key = |{ save_node_key }|.
          go_tree->expand_node( EXPORTING  node_key       = temp_save_node_key
...
*                WITH sy-msgv1 sy-msgv2 sy-msgv3 sy-msgv4.
  ...
  METHOD on_selection_changed.
    ...
    DATA diff_item   TYPE REF TO lcl_xlsx_diff_item.
    DATA item_2      TYPE lcl_xlsx_diff_item=>ty_diff_item.

    go_tree->node_get_item( EXPORTING  node_key       = node_key
                                       item_name      = 'C1'
                            IMPORTING  item           = item
                            EXCEPTIONS node_not_found = 1
                                       item_not_found = 2
...
*                WITH sy-msgv1 sy-msgv2 sy-msgv3 sy-msgv4.
    ENDIF.
    go_tree->node_get_user_object( EXPORTING  node_key       = node_key
...
CLASS lcl_xlsx_diff_file_ext IMPLEMENTATION.
  ...
  METHOD on_selection_changed.
    ...
    DATA downloaded_file_old TYPE string.
    ...
    TRY.
        ...
        CASE node-diff_state.
          WHEN state-added.
            ...
            downloaded_file_new = zip_get_and_gui_download( zip    = zip_new
                                                            name   = node-full_path
                                                            prefix = 'added' ).
          ...
          WHEN state-changed
            ...
            downloaded_file_old = zip_get_and_gui_download( zip    = zip_old
                                                            name   = node-full_path
                                                            prefix = 'old' ).
            downloaded_file_new = zip_get_and_gui_download( zip    = zip_new
                                                            name   = node-full_path
  ...
  METHOD gui_download.
    ...
    cl_gui_frontend_services=>gui_download( EXPORTING  bin_filesize            = xstrlen( i_content )
...
*                   WITH sy-msgv1 sy-msgv2 sy-msgv3 sy-msgv4.
  ...
  METHOD xml_pretty_print.
    ...
    CALL FUNCTION 'SDIXML_XML_TO_DOM'
      EXPORTING  xml      = c_content
      IMPORTING  document = xml_document
      EXCEPTIONS OTHERS   = 1.
    CALL FUNCTION 'SDIXML_DOM_TO_XML'
      EXPORTING  document      = xml_document
                 pretty_print  = abap_true
      IMPORTING  xml_as_string = c_content
      EXCEPTIONS OTHERS        = 2.
  ENDMETHOD.
  ...
  METHOD gui_execute.
    ...
    cl_gui_frontend_services=>execute( EXPORTING  application            = 'code'
    ...
    IF sy-subrc <> 0.
      ...
      RAISE EXCEPTION TYPE lcx_zip_diff
        EXPORTING text = temp_string.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD zip_get_and_gui_download.
    ...
    DATA new_parts   TYPE string_table.
    DATA part        TYPE REF TO string.
    ...
    result = |{ temp_dir }\\{ prefix }_{ concat_lines_of( table = new_parts
                                                          sep   = '_' ) }.xml|.
    IF name CS '.xml'.
...
CLASS lcl_app IMPLEMENTATION.
  ...
  METHOD gui_upload.
    ...
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_excel
        EXPORTING error = |gui_upload error { file_name }|.
    ENDIF.

    result = cl_bcs_convert=>solix_to_xstring( it_solix = solix_tab
                                               iv_size  = file_length ).
  ENDMETHOD.
  ...
  METHOD load_alv_table.
    ...
    DATA check_result TYPE ty_check_result.
    DATA cell_type    TYPE salv_s_int4_column.
    DATA error        TYPE REF TO cx_root.
  ...
  METHOD on_link_clicked.
    ...
    DATA error          TYPE REF TO cx_root.
    DATA zip_new        TYPE REF TO cl_abap_zip.
    DATA question       TYPE ty_popup_confirm_question.
    DATA refresh_stable TYPE lvc_s_stbl.
    ...
    TRY.
        ...
        CASE column.
            ...
            TRY.
                IF viewer IS NOT BOUND.
                  CREATE OBJECT viewer TYPE lcl_xlsx_diff_viewer
                    EXPORTING io_container = zip_diff_container.
                ENDIF.

                CREATE OBJECT zip_old.
                zip_old->load( EXPORTING  zip             = alv_line-compare_xlsx_reference
                               EXCEPTIONS zip_parse_error = 1
                                          OTHERS          = 2 ).
                IF sy-subrc <> 0.
                  RAISE EXCEPTION TYPE zcx_excel
                    EXPORTING error = |zip load old { alv_line-obj_text }|.
                ENDIF.

                CREATE OBJECT zip_new.
                zip_new->load( EXPORTING  zip             = alv_line-compare_xlsx_just_now
                               EXCEPTIONS zip_parse_error = 1
                                          OTHERS          = 2 ).
                IF sy-subrc <> 0.
                  RAISE EXCEPTION TYPE zcx_excel
                    EXPORTING error = |zip load new { alv_line-filename }|.
                ENDIF.

                viewer->diff_and_view( title_old = '"old" (Web Repository/expected)'
                                       zip_old   = zip_old
              ...
              CATCH cx_root INTO error.
                RAISE EXCEPTION TYPE zcx_excel
                  EXPORTING error = |Viewer error: { error->get_text( ) }|.
            ENDTRY.
          ...
          WHEN 'WRITE_SMW0'.
            ...
            write_xlsx_to_web_repository( objid    = alv_line-objid
  ...
  METHOD popup_confirm.
    ...
    CALL FUNCTION 'POPUP_TO_CONFIRM'
      EXPORTING text_question = question
      IMPORTING answer        = l_answer. "1 = button 1, 2 = button 2, A = cancel
    CASE l_answer.
      WHEN '2' OR 'A'.
        RAISE EXCEPTION TYPE zcx_excel
          EXPORTING error = 'Action cancelled by user'.
    ENDCASE.
  ENDMETHOD.
  ...
  METHOD read_xlsx_from_web_repository.
    ...
    DATA mime_table     TYPE TABLE OF w3mime.
    ...
    CALL FUNCTION 'WWW_GET_MIME_OBJECT'
      TABLES     query_string        = query_table
                 html                = html_table
                 mime                = mime_table
      CHANGING   return_code         = return_code
    ...
    ELSEIF sy-subrc >= 2.
      RAISE EXCEPTION TYPE zcx_excel
        EXPORTING error = 'WWW_GET_MIME_OBJECT'.
    ENDIF.

    result = cl_bcs_convert=>solix_to_xstring( it_solix = mime_table
                                               iv_size  = content_length ).
  ENDMETHOD.

  METHOD read_screen_fields.
    DATA lt_dummy   TYPE TABLE OF rsparams.
    DATA lt_sel_255 TYPE TABLE OF rsparamsl_255.
    ...
    CALL FUNCTION 'RS_REFRESH_FROM_SELECTOPTIONS'
      ...
      TABLES     selection_table     = lt_dummy
  ...
  METHOD screen_1001_pbo_first_time.
    ...
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_excel
        EXPORTING error = 'create splitter'.
    ENDIF.

    alv_container = splitter->get_container( row    = 1
                                             column = 1 ).
    zip_diff_container = splitter->get_container( row    = 1
                                                  column = 2 ).

    cl_salv_table=>factory( EXPORTING r_container  = alv_container
  ...
  METHOD write_xlsx_to_web_repository.
    ...
    DATA mime               TYPE TABLE OF w3mime.
    DATA key                TYPE wwwdatatab.
    DATA wwwparam           TYPE wwwparams.
    DATA table_of_wwwparams TYPE TABLE OF wwwparams.
    ...
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_excel
        EXPORTING error = |Object must be first created manually|.
    ENDIF.
    ...
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_excel
        EXPORTING error = 'WWWDATA_EXPORT'.
    ENDIF.
    ...
    CALL FUNCTION 'WWWPARAMS_UPDATE'
      TABLES     params       = table_of_wwwparams
      EXCEPTIONS update_error = 1
                 OTHERS       = 2.
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE zcx_excel
        EXPORTING error = |WWWPARAMS_UPDATE { objid } { filename }|.
    ENDIF.
  ENDMETHOD.
ENDCLASS.
